...
function Tile({ label, paisa, tone, children }: { label: string; paisa: number; tone: string; children: string }) {
  return (
    <div className="border-border bg-card flex flex-col gap-1 rounded-md border p-4">
            ...
            <tbody className="divide-hairline divide-y">
              {houses.map((house) => (
                <tr key={house.siteId}>
                  ...
                  <td className="text-muted-foreground py-2.5 pr-4 text-sm">{STAGE[house.stage] ?? house.stage}</td>
