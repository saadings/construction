...
export const record = siteMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // A buyer named once at the sale still becomes a person, because the money has to have come from somebody.
    ...
    if (fromId === undefined) {
      ...
      fromId = await ctx.db.insert('people', { name: receipt.newPerson, hidden: false })
    }
