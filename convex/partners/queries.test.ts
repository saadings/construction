...
// A house is sold, which is the moment a profit exists to be shared. Everything before that is capital at work.
async function sell(t: ReturnType<typeof convexWithPartners>, siteId: Id<'sites'>) {
  await t.run(async (ctx) => {
    await ctx.db.patch('sites', siteId, { stage: 'sold' })
  })
}
...
describe('a house that has not been sold', () => {
  it('owes nobody anything, however well it is doing', async () => {
    // "the shareholder profits are calculated when a house is sold". Until then the money in it is capital at work, and a share of a figure that has not happened is owed to nobody.
    const t = convexWithPartners()
    const house = await t.run(aHouseTwoPartnersPutInto)

    const read = await positionsOn(t, house.siteId)

    expect(read.sold).toBe(false)
    expect(read.positions.map((position) => position.duePaisa)).toEqual([0, 0])
    // And what each of them put in still reads, which is the figure that matters while it is being built.
    expect(read.positions.map((position) => position.capitalPaisa)).toEqual([6_000_000, 2_000_000])
  })

  it('says what a share would come to if it sold today, and keeps it out of what is owed', async () => {
    // Nauman: "Yes, clearly marked as an estimate." It is a guess about a house still being built, so it is not in the same shape as what somebody is owed -- one column cannot be read for the other if they are not in the same column.
    const t = convexWithPartners()
    const house = await t.run(aHouseTwoPartnersPutInto)

    const read = await positionsOn(t, house.siteId)

    expect(read.ifItSoldToday?.profitPaisa).toBe(3_500_000)
    expect(read.ifItSoldToday?.shares.map((share) => [share.name, share.paisa])).toEqual([
      ['The partner', 2_625_000],
      ['Another partner', 875_000],
    ])
    // The seam, asserted: nothing in a position carries the estimate.
    for (const position of read.positions) {
      expect(position.duePaisa).toBe(0)
      expect(Object.values(position)).not.toContain(2_625_000)
    }
  })

  it('drops the estimate the moment it is sold, so there are never two figures', async () => {
    const t = convexWithPartners()
    const house = await t.run(aHouseTwoPartnersPutInto)
    expect((await positionsOn(t, house.siteId)).ifItSoldToday).not.toBeNull()

    await sell(t, house.siteId)
    const read = await positionsOn(t, house.siteId)

    expect(read.ifItSoldToday).toBeNull()
    expect(read.positions.map((position) => position.duePaisa)).toEqual([2_625_000, 875_000])
  })

  it('still reads what has gone back to somebody, which is a real state', async () => {
    // Money out before the sale is money he has had and is not yet owed. It reads as that rather than being hidden.
    const t = convexWithPartners()
    const house = await t.run(aHouseTwoPartnersPutInto)
    ...
    expect(first.paidPaisa).toBe(2_000_000)
    expect(first.duePaisa).toBe(0)
    expect(first.balancePaisa).toBe(-2_000_000)
  })
})

describe('money that would otherwise leave the ledger', () => {
  it('shows somebody who has only ever been paid, with nothing owed to them', async () => {
    // A name mis-picked in a picker: `record` takes any person, and if the list of partners came only from capital and shares, the money would be stored, accepted, and on no screen and in no total.
    const t = convexWithPartners()
    const house = await t.run(async (ctx) => {
      const built = await aHouseTwoPartnersPutInto(ctx)
      const stranger = await ctx.db.insert('people', { name: 'Somebody else entirely', hidden: false })
    ...
    const read = await positionsOn(t, house.siteId)
    const stranger = read.positions.find((position) => position.name === 'Somebody else entirely')

    expect(stranger?.capitalPaisa).toBe(0)
    expect(stranger?.duePaisa).toBe(0)
    // A negative balance is exactly what "paid somebody who was owed nothing" is. It states the mistake rather than hiding it.
    expect(stranger?.balancePaisa).toBe(-2_000_000)
  })

  it('keeps a real partner on the list after his money is corrected away', async () => {
    // The one that happens without anybody typing anything wrong: a `moneyIn` row put right takes its owner out of the capital, and his payout would go with him. A correction to one record silently erasing another.
    const t = convexWithPartners()
    const house = await t.run(aHouseTwoPartnersPutInto)
    ...
    await t.run(async (ctx) => {
      const put = await ctx.db
        .query('moneyIn')
        .withIndex('bySiteAndDay', (q) => q.eq('siteId', house.siteId))
        .collect()

      for (const one of put) {
        if (one.fromId === house.other && one.why === 'partnerMoney') {
          await ctx.db.patch('moneyIn', one._id, { removed: true })
        }
      }
    })

    const read = await positionsOn(t, house.siteId)
    const stillThere = read.positions.find((position) => position.name === 'Another partner')

    expect(stillThere?.capitalPaisa).toBe(0)
    expect(stillThere?.paidPaisa).toBe(500_000)
    expect(stillThere?.balancePaisa).toBe(-500_000)
  })
})
