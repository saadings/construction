 ...
 * handler, ending in a status code.
 *
 * The status code is the whole point, and it is the part a unit test of the
 * handler cannot see. Clerk reads the response to decide what to do next:
 *
 ...
 *
 * `convex/webhooks/clerk.test.ts` already covers which mutations the switch
 * runs. Nothing here repeats that; these tests are about what comes back.
 */
...
/** A user.created payload with only the parts the handler reads. */
